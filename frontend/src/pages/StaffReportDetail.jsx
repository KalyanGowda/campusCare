...
export function StaffReportDetail({
    managementBasePath = "/api/staff/reports",
    backPath = "/staff/queue",
    backLabel = "Back to My Queue",
    rejectionHelp = "Use this only if the issue cannot be verified. Your reason is logged and visible to admin.",
}) {
    const { id } = useParams();
    ...
    const handleStatusUpdate = async () => {
        ...
        setUpdating(true);
        setUpdateMsg("");
        const res = await apiFetch(`${managementBasePath}/${id}/status`, {
            method: "PATCH",
    ...
    const handleReject = async () => {
        ...
        setRejecting(true);
        setRejectError("");
        const res = await apiFetch(`${managementBasePath}/${id}/reject`, {
            method: "PATCH",
    ...
    if (error || !report)
        return <div className="p-8 text-sm text-danger">{error || "Report not found."}</div>;
    const nextStatus = ALLOWED_NEXT[report.status];
    const canAct = !!nextStatus && report.status !== "Rejected";
    return (<div className="max-w-[1200px] flex flex-col">
      <button onClick={() => navigate(backPath)} className="inline-flex items-center text-[13px] font-medium text-[#1A2B4A] hover:underline mb-4">
        <ChevronLeft className="w-4 h-4 mr-0.5"/> {backLabel}
      </button>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          ...
          <div className="bg-white rounded-[10px] border border-[#E2E6EF] p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className={clsx("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold", report.status === "Open" && "bg-gray-100 text-gray-800", report.status === "Acknowledged" && "bg-blue-100 text-blue-800", report.status === "In Progress" && "bg-yellow-100 text-yellow-800", report.status === "Resolved" && "bg-green-100 text-green-800", report.status === "Rejected" && "bg-red-100 text-red-800")}>
                {report.status}
              </span>
            </div>
            ...
            {report.photo_url && (<>
                ...
                <div>
                  ...
                  <img src={`${API}${report.photo_url}`} alt="Report photo" className="rounded-lg max-h-64 object-cover border border-[#E2E6EF]"/>
                </div>
              </>)}
          ...
          <div className="bg-white rounded-[10px] border border-[#E2E6EF] p-6">
            ...
            {report.status_history.length === 0 ? (<p className="text-sm text-[#6B7280]">No history yet.</p>) : (<div className="relative pl-4 space-y-6">
                <div className="absolute left-5 top-2 bottom-2 w-px bg-[#E2E6EF]"/>
                {report.status_history.map((h, i) => (<div key={i} className="relative flex items-start pl-6">
        ...
        <div className="w-full lg:w-[340px] flex flex-col gap-4 flex-shrink-0">
          ...
          {/* Update Status card */}
          {canAct && (<div className="bg-white rounded-[10px] border border-[#E2E6EF] p-5">
              ...
              <div className="mb-4">
                ...
                <span className={clsx("inline-flex items-center px-4 py-2 rounded-full text-sm font-medium", report.status === "Open" && "bg-gray-100 text-gray-800", report.status === "Acknowledged" && "bg-blue-100 text-blue-800", report.status === "In Progress" && "bg-yellow-100 text-yellow-800")}>
                  {report.status}
                </span>
              </div>
              ...
              <button onClick={handleStatusUpdate} disabled={updating} className="w-full bg-[#1A2B4A] text-white text-[14px] font-semibold py-[11px] rounded-md hover:opacity-90 transition-opacity disabled:opacity-50">
          ...
          {canAct && report.status !== "Resolved" && (<div className="bg-white rounded-[10px] border border-[#FCA5A5] p-5">
              ...
              <label className="block mb-3">
                ...
                <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)} className="w-full h-[96px] rounded-md border border-[#E2E6EF] p-3 text-[13px] resize-none focus:outline-none focus:border-red-400" placeholder="e.g. Visited Room 204 — issue not reproducible."/>
              </label>
              {rejectError && <p className="text-xs text-danger mb-2">{rejectError}</p>}
              <button onClick={handleReject} disabled={rejecting} className="w-full bg-transparent border border-[#DC2626] text-[#DC2626] text-[13px] font-semibold py-[10px] rounded-md hover:bg-red-50 transition-colors disabled:opacity-50">
