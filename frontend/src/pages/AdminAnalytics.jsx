...
export function AdminAnalytics() {
    ...
    if (loading)
        return <p className="text-sm text-ink-muted p-6">Loading analytics…</p>;
    if (!data)
        return <p className="text-sm text-danger p-6">Failed to load analytics.</p>;
    ...
    return (<div className="flex flex-col min-w-0 max-w-full gap-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-1">
        ...
        <div className="bg-white rounded-[10px] border border-[#E2E6EF] p-5">
          ...
          {data.by_block.length === 0 ? (<p className="text-sm text-[#6B7280]">No data yet.</p>) : (<div className="flex flex-col gap-2">
              {data.by_block.map(row => (<div key={row.block_name} className="flex items-center">
        ...
        <div className="bg-white rounded-[10px] border border-[#E2E6EF] p-5">
          ...
          {data.by_category.length === 0 ? (<p className="text-sm text-[#6B7280]">No data yet.</p>) : (<div className="flex flex-col gap-2">
              {data.by_category.map(row => (<div key={row.sub_type} className="flex items-center">
        ...
        <div className="bg-white rounded-[10px] border border-[#E2E6EF] p-5 lg:col-span-2">
          ...
          {total === 0 ? (<p className="text-sm text-[#6B7280]">No reports this month yet.</p>) : (<div className="flex flex-col md:flex-row items-center gap-10">
              <div className="w-[200px] h-[200px] rounded-full relative flex-shrink-0 flex items-center justify-center" style={{
                background: total > 0
