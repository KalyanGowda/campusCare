...
export function AdminAllReports() {
    ...
    const formatLocation = (r) => r.room_number ? `Room ${r.room_number}` : r.landmark_name ?? r.location_type;
    ...
    return (<div className="flex flex-col min-w-0 max-w-full">
      ...
      <div className="bg-white rounded-[10px] border border-[#E2E6EF] p-4 flex gap-3 mb-4 items-center flex-wrap xl:flex-nowrap">
        <div className="relative w-[160px]">
          <select value={blockFilter} onChange={e => setBlockFilter(e.target.value)} className="appearance-none w-full h-[38px] px-3 pr-8 rounded-md border border-[#E2E6EF] text-[13px] bg-white focus:outline-none focus:border-navy">
        ...
        <div className="relative w-[160px]">
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="appearance-none w-full h-[38px] px-3 pr-8 rounded-md border border-[#E2E6EF] text-[13px] bg-white focus:outline-none focus:border-navy">
        ...
        <div className="relative flex-grow min-w-[200px]">
          ...
          <input type="text" placeholder="Search by category" value={search} onChange={e => setSearch(e.target.value)} className="w-full h-[38px] pl-9 pr-3 rounded-md border border-[#E2E6EF] text-[13px] bg-white focus:outline-none focus:border-navy"/>
        </div>
      </div>

      {escalated > 0 && (<div className="bg-[#FFF5F5] border border-[#FCA5A5] rounded-lg px-5 py-[14px] flex justify-between items-center mb-4">
      ...
      <div className="bg-white rounded-[10px] border border-[#E2E6EF] overflow-hidden">
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead>
              <tr className="bg-[#F9FAFB] border-b border-[#E2E6EF]">
                {['ID', 'Location', 'Block', 'Category', 'Reported By', 'Status', 'Date', 'Assigned To', ''].map(h => (<th key={h} className="h-[48px] px-3 text-[12px] font-medium text-[#6B7280] uppercase tracking-[0.04em]">{h}</th>))}
              </tr>
            </thead>
            <tbody>
              {loading ? (<tr><td colSpan={9} className="py-10 text-center text-sm text-[#6B7280]">Loading…</td></tr>) : reports.length === 0 ? (<tr><td colSpan={9} className="py-10 text-center text-sm text-[#6B7280]">No reports found.</td></tr>) : reports.map((row) => (<tr key={row.id} onClick={() => navigate(`/admin/reports/${row.id}`)} className="border-b border-[#E2E6EF] hover:bg-gray-50 h-[60px] cursor-pointer">
                  ...
                  <td className="px-3"><button onClick={(event) => { event.stopPropagation(); navigate(`/admin/reports/${row.id}`); }} aria-label={`View report ${row.id}`} className="p-1.5 text-[#1A2B4A] hover:bg-slate-100 rounded"><Eye className="w-4 h-4" /></button></td>
                </tr>))}
            </tbody>
          </table>
        </div>
        <div className="h-[48px] bg-[#F9FAFB] px-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            {Array.from({ length: Math.min(pages, 5) }, (_, i) => i + 1).map(p => (<button key={p} onClick={() => setPage(p)} className={clsx("w-7 h-7 rounded-md text-sm font-medium flex items-center justify-center", page === p ? "bg-[#1A2B4A] text-white" : "text-gray-600 hover:bg-gray-100")}>
                {p}
              </button>))}
