...
export function DashboardShell({ role, userName: userNameProp, userLabel: userLabelProp, navItems, children, pageTitle, }) {
    ...
    return (<div className="flex h-screen w-full font-sans bg-page-bg overflow-hidden">
      {/* Sidebar */}
      <div className="w-[220px] bg-navy flex flex-col h-full shrink-0">
        ...
        <nav className="flex-1 px-3 flex flex-col gap-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (<Link key={item.path} to={item.path} className={clsx("flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-all", isActive
                ...
                {isActive && (<div className="absolute left-0 w-[3px] h-8 bg-accent rounded-r-full"/>)}
        ...
        <div className="p-3 shrink-0">
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-white/70 hover:text-white hover:bg-navy-deep transition-all">
