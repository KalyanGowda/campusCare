...
export function StaffQueue() {
  ...
  const fetchQueue = () => {
    ...
    apiFetch('/api/staff/block-rating')
      .then(r => r.json())
      .then(r => { if (r && typeof r.final_rating === 'number') setRating(r) })
      .catch(() => {})
  }
  ...
  const slaLabel = (r: Report) => {
    if (r.sla_status === 'overdue') return '🔴 Overdue'
    if (r.sla_status === 'warning') return `🟡 ${Math.round(r.hours_remaining)}h left`
    return `🟢 ${Math.round(r.hours_remaining)}h left`
  }
  const slaColor = (r: Report) => {
    if (r.sla_status === 'overdue') return 'text-[#DC2626]'
    if (r.sla_status === 'warning') return 'text-[#D97706]'
    return 'text-[#16A34A]'
  }
  ...
  return (
    <div className="flex flex-col gap-6 lg:flex-row lg:items-start max-w-[1200px]">
        ...
        <div className="flex justify-between items-center mb-5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-[240px] h-[38px] pl-9 pr-3 rounded-md border border-[#E2E6EF] text-sm focus:outline-none focus:border-navy"
            />
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            {tabs.map((tab) => (
              <button
                ...
                className={clsx(
                  "px-[14px] py-1.5 rounded-md text-[13px] font-medium font-inter transition-colors",
                  activeTab === tab ? "bg-[#1A2B4A] text-white" : "bg-white text-[#6B7280] border border-[#E2E6EF] hover:bg-gray-50"
                )}
              >
                {tab}
              </button>
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none w-[160px] h-[38px] px-3 pr-8 rounded-md border border-[#E2E6EF] text-sm text-gray-700 bg-white focus:outline-none focus:border-navy"
            >
        ...
        <div className="bg-white rounded-[10px] border border-[#E2E6EF] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left whitespace-nowrap">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={7} className="py-10 text-center text-sm text-[#6B7280]">Loading…</td></tr>
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={7} className="py-10 text-center text-sm text-[#6B7280]">No reports found.</td></tr>
                ) : (
                  filtered.map((row) => (
                    <tr key={row.id} className="border-b border-[#E2E6EF] hover:bg-[#EFF6FF] h-[64px]">
