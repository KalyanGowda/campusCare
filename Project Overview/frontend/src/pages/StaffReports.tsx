...
export function StaffReports() {
  ...
  const filtered = search
    ? reports.filter(r =>
        formatLocation(r).toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1200px]">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-[280px] h-[38px] pl-9 pr-3 rounded-md border border-[#E2E6EF] text-sm focus:outline-none focus:border-navy"
          />
        </div>
      </div>
      ...
      {/* Reports Grid */}
      {loading ? (
        <div className="text-center py-10 text-sm text-[#6B7280]">Loading reports…</div>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-[10px] border border-[#E2E6EF] p-12 text-center">
          <FileText className="w-12 h-12 text-gray-300 mx-auto mb-3" />
          <p className="text-sm text-[#6B7280]">No reports found.</p>
          {search && (
            <button
              onClick={() => setSearch("")}
              className="mt-2 text-sm text-[#1A2B4A] hover:underline"
            >
              Clear search
            </button>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((report) => (
            <div
              key={report.id}
              className="bg-white rounded-[10px] border border-[#E2E6EF] p-5 hover:shadow-md transition-shadow cursor-pointer"
            ...
            >
              ...
              {parseInt(String(report.confirmation_count)) > 0 && (
                <div className="flex items-center gap-1 text-[12px] font-medium text-[#E8A020] mb-3">
                  <span>✓</span>
              ...
              <button
                className="w-full text-[13px] font-semibold text-[#1A2B4A] hover:underline text-center"
                onClick={(e) => {
                  e.stopPropagation()
