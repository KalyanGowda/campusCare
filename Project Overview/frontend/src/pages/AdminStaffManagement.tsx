  ...
  useEffect(() => {
    ...
    apiFetch('/api/admin/block-ratings').then(r => r.json()).then(data => {
      if (Array.isArray(data)) {
        setBlocks(data.map((b: { block_id: number; block_name: string }) => ({ id: b.block_id, name: b.block_name })))
      }
    })
  }, [])
    ...
    <div className="flex flex-col min-w-0 max-w-full">
      <div className="flex justify-between items-center mb-5">
        ...
        <button onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 px-5 py-2.5 bg-[#1A2B4A] text-white text-[13px] font-semibold font-inter rounded-md hover:opacity-90 transition-opacity">
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead>
              <tr className="bg-[#F9FAFB] border-b border-[#E2E6EF]">
                {['Name','Email','Assigned Block','Reports Handled','Account Created','Actions'].map(h => (
                  <th key={h} className="h-[48px] px-4 text-[12px] font-medium text-[#6B7280] uppercase tracking-[0.04em]">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="py-10 text-center text-sm text-[#6B7280]">Loading…</td></tr>
              ) : staff.length === 0 ? (
                <tr><td colSpan={6} className="py-10 text-center text-sm text-[#6B7280]">No staff accounts yet.</td></tr>
              ) : staff.map((row) => (
                <tr key={row.id} className="border-b border-[#E2E6EF] hover:bg-gray-50 h-[68px]">
                  <td className="px-4">
                  ...
                  <td className="px-4 text-[13px] font-inter">
                    <button onClick={() => handleRemove(row.id, row.name)}
                      className="text-[#DC2626] font-semibold hover:underline">Remove</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="bg-white rounded-[12px] shadow-2xl w-full max-w-[480px] p-8">
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type} placeholder={f.placeholder}
                    ...
                    className="w-full h-[40px] px-3 rounded-md border border-[#E2E6EF] text-[14px] bg-white focus:outline-none focus:border-navy" />
                </div>
              ))}
              <div>
                <label className="block text-[13px] font-medium text-[#6B7280] font-inter mb-1.5">Assign Block</label>
                <select value={form.block_id} onChange={e => setForm(prev => ({ ...prev, block_id: e.target.value }))}
                  className="w-full h-[40px] px-3 rounded-md border border-[#E2E6EF] text-[14px] bg-white focus:outline-none focus:border-navy appearance-none">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setIsModalOpen(false)}
                className="px-5 py-2.5 border border-[#E2E6EF] text-[#6B7280] text-[13px] font-medium font-inter rounded-md hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={handleAdd} disabled={submitting}
                className="px-5 py-2.5 bg-[#1A2B4A] text-white text-[13px] font-medium font-inter rounded-md hover:opacity-90 disabled:opacity-50">
