    ...
    return (<div className="grid max-w-[1200px] grid-cols-1 gap-6 xl:grid-cols-[420px_minmax(0,1fr)] xl:items-start">
      <div className="flex flex-col gap-6">
        ...
        <Section title="Profile">
          <form className="mt-5 space-y-4" onSubmit={saveProfile}>
            <label className="block">
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} className="h-10 w-full rounded-md border border-line px-3 text-sm text-ink outline-none focus:border-[1.5px] focus:border-navy"/>
            </label>
            <label className="block">
        ...
        <Section title="Notification Preferences">
          <div className="mt-2">
            {preferenceRows.map(([label, description], index) => (<div key={label} className={clsx("flex min-h-11 items-center justify-between gap-4 py-3", index !== preferenceRows.length - 1 && "border-b border-line")}>
                <div>
                  <p className="text-sm font-medium text-ink">{label}</p>
                  <p className="mt-0.5 text-xs text-ink-muted">{description}</p>
                </div>
                <Toggle label={label} enabled={preferences[index]} onChange={() => setPreferences(v => v.map((val, i) => i === index ? !val : val))}/>
              </div>))}
          </div>
        </Section>
      ...
      <Section title="Account">
        ...
        ].map(([label, value]) => (<div key={label} className="flex items-center justify-between gap-4 text-sm">
        ...
        <div className="pt-5">
          ...
          <button type="button" className="mt-3 w-full rounded-md border border-danger px-4 py-2 text-[13px] font-medium text-danger transition-colors hover:bg-danger/5">
            Delete Account
          </button>
