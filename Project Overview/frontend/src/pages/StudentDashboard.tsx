    ...
    <div className="flex gap-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-line overflow-hidden mt-2">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-line">
              {loading ? (
                <tr><td colSpan={5} className="py-10 text-center text-sm text-ink-muted">Loading…</td></tr>
              ) : recent.length === 0 ? (
                <tr><td colSpan={5} className="py-10 text-center text-sm text-ink-muted">No reports yet. Submit your first one!</td></tr>
              ) : (
                recent.map((report) => (
                  <tr key={report.id} className="hover:bg-page-bg/30 transition-colors">
      ...
      <div className="w-[320px] flex flex-col gap-6 shrink-0">
        <div className="bg-navy rounded-xl p-6 text-white shadow-sm flex flex-col items-center text-center gap-4">
          ...
          <Link
            to="/student/report"
            className="w-full bg-accent hover:bg-[#D98E16] text-navy font-semibold py-2.5 rounded-md transition-colors"
          >
            Report Problem
          </Link>
        </div>
