...
interface Report {
  ...
  status_history?: { new_status: string; changed_at: string }[]
}
...
export function MyReports() {
  ...
  const formatDateTime = (iso: string) => new Date(iso).toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })
    ...
    <div className="flex flex-col gap-6 w-full max-w-4xl relative">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-xl border border-line shadow-sm">
        <div className="flex items-center gap-2 overflow-x-auto">
          {["All", "Open", "Acknowledged", "In Progress", "Resolved", "Rejected"].map((s) => (
            <button
              key={s}
              onClick={() => setSelectedStatus(s)}
              className={clsx(
                "px-4 py-1.5 rounded-full text-sm font-medium transition-colors whitespace-nowrap",
                selectedStatus === s ? "bg-navy text-white" : "bg-page-bg text-ink-muted hover:bg-line",
              )}
            >
              {s}
            </button>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-1.5 border border-line rounded-md text-sm outline-none focus:border-navy"
            />
          </div>
          ...
          filtered.map((report) => {
            ...
            return (
              <div
                ...
                className={clsx("bg-white rounded-xl shadow-sm border border-line overflow-hidden border-l-4 cursor-pointer hover:shadow-md transition-shadow", borderClass)}
              >
                <div className="p-5 flex flex-col gap-3">
                  ...
                  <div className="flex justify-between items-center mt-2 border-t border-line pt-3">
                    ...
                    {report.status === "Open" && (
                      <button
                        onClick={e => { e.stopPropagation(); cancelReport(report.id) }}
                    ...
                    {report.status === "Resolved" && (
                      <button className="text-success text-xs font-medium flex items-center gap-1 hover:underline">
            ...
            <div className="p-6 flex-1 overflow-y-auto flex flex-col gap-6">
              {drawerLoading ? (
                ...
                <>
                  ...
                  {drawerReport.status_history && drawerReport.status_history.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-col relative ml-2">
                        ...
                        {[...drawerReport.status_history].reverse().map((h, i) => (
                          <div key={i} className="flex gap-4 relative z-10 pb-6">
