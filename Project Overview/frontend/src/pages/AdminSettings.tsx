...
interface UserData { id: number; name: string; email: string; role: string; created_at: string }
interface Analytics { total_this_month: number }
...
export function AdminSettings() {
  ...
  const saveSla = (e: FormEvent) => { e.preventDefault(); setSlaSaved(true); setTimeout(() => setSlaSaved(false), 3000) }
        ...
        <Section title="Profile">
          <form className="mt-5 space-y-4" onSubmit={saveProfile}>
            ...
            <label className="block">
              ...
              <input type="text" value="BMSIT&M" disabled
                className="h-10 w-full rounded-md border border-line px-3 text-sm bg-[#F3F4F6] text-[#9CA3AF] outline-none" />
            </label>
        ...
        <div className="rounded-[10px] border border-[#E2E6EF] bg-white p-6 shadow-sm">
          ...
          <form onSubmit={saveSla}>
            <div className="flex flex-col mb-6">
              {slaRows.map((row, i) => (
                <div key={row.label} className={clsx("flex justify-between items-center h-[44px]", i !== slaRows.length - 1 && "border-b border-[#E2E6EF]")}>
                  <span className="text-[14px] text-[#1C2333]">{row.label}</span>
                  <div className="flex items-center gap-2">
                    <input type="text" defaultValue={row.value}
                      className="w-[64px] h-[36px] rounded-md border border-[#E2E6EF] text-center text-[14px] font-semibold text-[#1C2333] focus:outline-none focus:border-navy" />
