    ...
    return (<div className="w-full max-w-[1200px]">
      <div className="mb-5 flex items-end justify-between gap-4 border-b border-line">
        <div className="flex items-center gap-6">
          {["All", "Unread", "Read"].map((tab) => (<button key={tab} type="button" onClick={() => setFilter(tab)} className={clsx("border-b-2 pb-3 text-sm transition-colors", filter === tab ? "border-accent font-semibold text-navy" : "border-transparent font-normal text-ink-muted hover:text-navy")}>
              {tab}
            </button>))}
        </div>
      ...
      <div className="overflow-hidden rounded-[10px] border border-line bg-white shadow-sm">
        {loading ? (<div className="flex min-h-72 flex-col items-center justify-center">
            <p className="text-sm text-ink-muted">Loading…</p>
          </div>) : visible.length === 0 ? (<div className="flex min-h-72 flex-col items-center justify-center px-6 text-center">
            ...
            <p className="mt-1 text-[13px] text-ink-muted">You'll be notified when your reports are updated.</p>
          </div>) : (visible.map((item, index) => (<div key={item.id} onClick={() => !item.is_read && markOneRead(item.id)} className={clsx("flex min-h-[72px] gap-3 px-5 py-4 transition-colors hover:bg-page-bg/40 cursor-pointer", index !== visible.length - 1 && "border-b border-line")}>
              ...
              <div className="min-w-0 flex-1">
                ...
                {item.subtitle && (<p className="mt-1 truncate text-[13px] text-ink-muted">{item.subtitle}</p>)}
                {item.type === "resolved" && (<button type="button" className="mt-2 text-xs font-medium text-success hover:underline">Leave Feedback</button>)}
              </div>
